...
class Clock : Fragment() {
    ...
    private fun calculateAndUpdateData(latitude: Double, longitude: Double) {
        CoroutineScope(Dispatchers.Default).launch {
            ...
            val sunTimeData = fromHtml("<b>${getString(R.string.sun_sunrise)}</b> ${pattern.format(sunTimes.rise)}<br>" +
            ...
            val moonTimeData = fromHtml("<b>${getString(R.string.moon_moonrise)}</b> ${pattern.format(moonTimes.rise)}<br>" +
            ...
            val twilightData = fromHtml("<b>${getString(R.string.twilight_astronomical_dawn)}</b> ${pattern.format(twilightTimes.twilight(SunTimes.Twilight.ASTRONOMICAL).execute().rise)}<br>" +
            ...
            val sunPositionData = fromHtml("<b>${getString(R.string.sun_azimuth)}</b> ${round(sunPosition.azimuth, 2)}° ${getDirectionCodeFromAzimuth(requireContext(), sunPosition.azimuth)}<br>" +
            ...
            val moonPositionData = fromHtml("<b>${getString(R.string.moon_azimuth)}</b> ${round(moonPosition.azimuth, 2)}° ${getDirectionCodeFromAzimuth(requireContext(), moonPosition.azimuth)}<br>" +
            ...
            val moonIlluminationData = fromHtml("<b>${getString(R.string.moon_fraction)}</b> ${round(moonIllumination.fraction, 2)}<br>" +
            ...
            val moonDatesData = fromHtml("<b>${getString(R.string.moon_full_moon)}</b> ${formatMoonDate(MoonPhase.compute().timezone(timezone).on(Instant.now()).phase(MoonPhase.Phase.FULL_MOON).execute().time)}<br>" +
    ...
    private fun updateTimeData() {
        CoroutineScope(Dispatchers.Default).launch {
            ...
            try {
                ...
                localTimeData = fromHtml("<b>${getString(R.string.local_timezone)}</b> ${zonedDateTime.zone}<br>" +
